...
def check_args():
    ...
    returns the .asc filename from the command line inputs
    """
    # check for correct command-line inputs
    if len(sys.argv) != 2:
        print "usage: python makeasc.py [original.asc file]"
        exit(-1)
...
SPACE = ' '
METADATA_HEADER = '**'
FIRST_EXCESS_TRIAL = 'TRIAL 101'
CALIBRATION_SIGNAL = '!CAL'
MSG_SIGNAL = 'MSG'
INPUT_SIGNAL = 'INPUT'
TRIALID_SIGNAL = 'TRIALID'
PREPARE_SEQUENCE_SIGNAL = 'prepare_sequence'
DISPLAY_COORDS_SIGNAL = 'DISPLAY_COORDS'
RETRACE_INTERVAL_SIGNAL = 'RETRACE_INTERVAL'
PROCESSED_OUTPUT_FOLDER_NAME = '/processed_asc/'
PROCESSED_FILE_EXTENSION = '_processed.asc'
RECORD_SIGNAL = '!MODE RECORD'
START_SIGNAL = 'START'
END_SIGNAL = 'END'
EVENTS_SIGNAL = 'EVENTS'
SFIX_SIGNAL = 'SFIX'
EFIX_SIGNAL = 'EFIX'
BUTTON_SIGNAL = 'BUTTON'
QUESTION_SIGNAL = 'SHOW FOLLOWUP QUESTION'
...
def open_input(original_asc):
    ...
    try:
        ...
        # skip everything after FIRST_EXCESS_TRIAL
        indexes = [idx for idx, s in enumerate(remaining_lines) if FIRST_EXCESS_TRIAL in s]
        if len(indexes) > 0:
        ...
        print("original.asc file not found or path is incorrect")
        exit(-1)
...
def conversion_metadata(buffer, remaining_lines):
    ...
    while not done:
        ...
        elif line.strip() == EMPTY_STRING:
            buffer.append(line)
        # keep all comments, which begin with **
        elif line[0:2] == METADATA_HEADER:
            buffer.append(line)
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and CALIBRATION_SIGNAL in line:
            done = True
            buffer.append(line)
        # keep all blank lines
        elif line.strip() == EMPTY_STRING:
        ...
        elif MSG_SIGNAL in line and DISPLAY_COORDS_SIGNAL in line:
            buffer.append(line)
        ...
        elif RETRACE_INTERVAL_SIGNAL in line:
            # pass
            buffer.append(line)
        # keep any input info
        elif INPUT_SIGNAL in line:
            buffer.append(line)
...
def calibration_validation(buffer, remaining_lines, recalibration):
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and TRIALID_SIGNAL in line:
            done = True
...
def practice_trials(buffer, remaining_lines):
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and PREPARE_SEQUENCE_SIGNAL in line:
            done = True
...
def skip_to_next_trial(buffer, remaining_lines):
    ...
    while not done:
        ...
        if MSG_SIGNAL in line and TRIALID_SIGNAL in line:
            ...
            trial_metadatas['buffer_holder_index_trialid_limerick'] = len(buffer)
            buffer.append(line)
        elif MSG_SIGNAL in line and CALIBRATION_SIGNAL in line:
...
def read_camera_info(buffer, remaining_lines):
    ...
    while not done:
        ...
        if 'RECCFG' in line or \
            ...
            RECORD_SIGNAL in line or \
            START_SIGNAL in line:
            line_split = line.split(None, 2)
            timestamp = int(line_split[1])
            line = line_split[0] + SPACE + str(timestamp + 600) + SPACE + line_split[2]

        # exit state
        if RECORD_SIGNAL in line:
            done = True
            try:
            ...
            buffer.append(line)
        elif START_SIGNAL in line and EVENTS_SIGNAL in line:
            buffer_holder_index_moder = len(buffer)
            buffer.append("placeholder for !MODE RECORD")
            buffer.append(line)
        elif INPUT_SIGNAL in line and '127' in line:
...
def skip_dual_task_begin_instructions(buffer, remaining_lines):
    ...
    while not done:
        ...
        # exit state
        if BUTTON_SIGNAL in line and str(line.strip().split()[-1]) == '1':
            done = True
            buffer.append(line)
            buffer.append(MSG_SIGNAL + SPACE + line.split(None, 3)[1] +' GAZE TARGET ON\n')
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and 'SHOW LIMERICK' in line:
            done = True
            line_split = line.split(None, 3)
            trial_metadatas['synctime'] = line_split[1]
            buffer.append(line_split[0] + SPACE + trial_metadatas.get('synctime') +' GAZE TARGET OFF\n')
            buffer.append(line_split[0] + SPACE + trial_metadatas.get('synctime') +' DISPLAY ON\n')
        elif 'IAREA FILE' in line:
            trial_metadatas['iarea'] = line
        elif MSG_SIGNAL in line and 'SYNCTIME' in line:
            line_split = line.split(None, 3)
            buffer.append(line_split[0] + SPACE + trial_metadatas.get('synctime') +' SYNCTIME\n')
        # collect eyetracking data, since we're still in gazetarget area
        elif SFIX_SIGNAL in line or \
                EFIX_SIGNAL in line or \
                'SSACC' in line or \
                'ESACC' in line or \
                'SBLINK' in line or \
                'EBLINK' in line or \
                BUTTON_SIGNAL in line:
            buffer.append(line)
...
def eye_movements_limerick(buffer, remaining_lines):
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and 'STOP SECONDARY TASK' in line:
            done = True
            trial_metadatas['timestamp_end_lim'] = str(line.split()[1])
            # mark trial ok at the end of the limerick portion, add placeholders
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' ENDBUTTON ' + button_number + '\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' DISPLAY OFF\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' TRIAL_RESULT ' + button_number +'\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' TRIAL OK\n')
            # put this placeholder here
            trial_metadatas['buffer_holder_index_eventsr'] = len(buffer)
            buffer.append(END_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + '\n')
...
def skip_dual_task_end_instructions(buffer, remaining_lines):
    ...
    while not done:
        ...
        # exit state
        if MSG_SIGNAL in line and QUESTION_SIGNAL in line:
            done = True
        elif BUTTON_SIGNAL in line:
            buffer.append(line)
...
def question_placeholders(buffer, remaining_lines):
    ...
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' TRIALID\n')

    # placeholder index for the .ias stuff that will go here
    trial_metadatas['buffer_holder_index_questionanswer'] = len(buffer)
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' QUESTION_ANSWER\n')
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' DELAY 0 MS\n')
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' DISPLAY TEXT 1\n')
...
def question_cam_ias(buffer, remaining_lines):
    ...
    for line in fake_question_ias:
        line_split = line.split(None, 2)
        timestamp = int(line_split[1])
        line = line_split[0] + SPACE + str(int(trial_metadatas.get('timestamp_end_lim')) + timestamp) + SPACE + line_split[2]
        buffer.append(line)
    ...
    for line in trial_metadatas.get('camera_info'):
        ...
        if 'RECCFG' in line or \
            ...
            line_split = line.split(None, 2)
            timestamp = int(line_split[1])
            line = line_split[0] + SPACE + str(int(trial_metadatas.get('timestamp_end_lim')) + 300) + SPACE + line_split[2]
            buffer.append(line)
        else:
            buffer.append(line)
...
def eye_movements_question(buffer, remaining_lines):
    ...
    """
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' DISPLAY ON\n')
    buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + ' SYNCTIME\n')
    ...
    while not done:
        ...
        # exit state
        if END_SIGNAL in line and EVENTS_SIGNAL in line and 'RES' in line:
            done = True
            trial_metadatas['timestamp_end_ques'] = str(line.split()[1])
            # mark trial ok at the end of the limerick portion, add placeholders
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_ques') + ' ENDBUTTON ' + button_number + '\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_ques') + ' DISPLAY OFF\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_ques') + ' TRIAL_RESULT ' + button_number +'\n')
            buffer.append(MSG_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_ques') + ' TRIAL OK\n')
...
def tweak_stuff(buffer, remaining_lines):
    ...
    EID = E + I + D
    new_id = trial_metadatas.get('old_trialid').rsplit(None, 1)[0] + SPACE + EID + '\n'
    buffer[trial_metadatas.get('buffer_holder_index_trialid_limerick')] = new_id


    # (for limerick) tweak the END EVENTS RES line
    buffer[trial_metadatas.get('buffer_holder_index_eventsr')] = END_SIGNAL + SPACE + trial_metadatas.get('timestamp_end_lim') + SPACE + trial_metadatas.get('events_res_line').split(None, 2)[2]

    # (for question) update the dirtytype question_answer
    buffer[trial_metadatas.get('buffer_holder_index_questionanswer')]  = buffer[trial_metadatas.get('buffer_holder_index_questionanswer')].strip() + SPACE + trial_metadatas.get('dirtytype') + '\n'

    # (for question) add + tweak the TRIALID
    EID = 'E100' + I + 'D1'
    buffer[trial_metadatas.get('buffer_holder_index_trialid_question')] = buffer[trial_metadatas.get('buffer_holder_index_trialid_question')].strip() + SPACE + EID + '\n'
...
def read_ias_letter(line, timestamp):
    ...
    buffer_slice.append(MSG_SIGNAL + SPACE + str(timestamp) + ' DISPLAY TEXT 1\n')
    ...
    while True:
        ...
        for i in range(len(word)+1):
            ...
            # add to buffer
            buffer_slice.append(MSG_SIGNAL + SPACE + str(timestamp) + ' REGION CHAR ' + str(char_number) + SPACE + str(line_number) + SPACE + c + SPACE + str(x_start) + SPACE + str(y_start) + SPACE + str(x) + SPACE + str(y_end) + '\n')
            buffer_slice.append(MSG_SIGNAL + SPACE + str(timestamp) + ' DELAY 1 MS' + '\n')
...
def write_to_outfile(buffer):
    ...
    new_asc_filename = inpath[0] + PROCESSED_OUTPUT_FOLDER_NAME + inpath[-1].split('.')[0] + PROCESSED_FILE_EXTENSION
